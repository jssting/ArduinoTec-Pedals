...
bool dbg_PrintBr = false;
bool dbg_PrintCTH = true;
...
void loop() {
  ...
  if (debug) {
    if(dbg_PrintThr){      
      String values = "Throttle Values: " + String(actThrVal) + ", max value: " + String(maxThr);
    ...
    if(dbg_PrintCTH){
    //  blCth = get_baseline(A2, 25);
      String values = "Baseline: " + String(blCth) + ", Clutch Values: " + String(actCthVal) + ", max value: " + String(maxCth);
      Serial.println(values);
      delay(100);}  
  }

  if (actBrkVal > (maxBrk * 0.80)) digitalWrite(3, LOW); //enable the motor to spin when the pedal reaches 80% of the pedal pressure
  else digitalWrite(3, HIGH); //stop the motor when it falls below 80%
